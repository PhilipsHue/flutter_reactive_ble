...
import com.signify.hue.flutterreactiveble.ble.*
import com.signify.hue.flutterreactiveble.model.*
import java.util.UUID
import com.signify.hue.flutterreactiveble.ProtobufModel as pb

@Suppress("TooManyFunctions", "NoWildcardImports")
class ProtobufMessageConverter {
    ...
    fun convertedConnectedDevicesInfo(
        result: FetchConnectedDevicesResult
    ): pb.ConnectedDevicesInfo {
        when (result) {
            is FetchConnectedDevicesSuccess -> {
                val protobufDevices = result.connectedDevices.map { device ->
                    pb.ConnectedDevice.newBuilder()
                        .setDeviceId(device.deviceId)
                        .setDeviceName(device.name).build()
                }

                return pb.ConnectedDevicesInfo.newBuilder()
                    .addAllConnectedDevices(protobufDevices).build()
            }
            is FetchConnectedDevicesFailure -> {
                val failure = pb.GenericFailure.newBuilder()
                    .setCode(0)
                    .setMessage(result.errorMessage)
                    .build()
    ...
    private fun fromBluetoothGattService(gattService: BluetoothGattService): pb.DiscoveredService {
        return pb.DiscoveredService.newBuilder()
            ...
            .addAllCharacteristics(gattService.characteristics.map {
                ...
                val writeNoResp =
                    (prop and BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE) > 0
